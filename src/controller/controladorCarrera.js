    ...
    await ModeloCarrera.create(carrera)
      ...
      .catch((error) => {
        ...
        if (error instanceof UniqueConstraintError) {
          ...
          let aux = (error.message).split(/[«»]+/); // primero extrae lo que está dentro de las comillas. 
          let msj = aux[1].split("_"); // extrae lo separado por guion bajo (carrera_columna_key) siendo msj[1] el valor columna
          mensajes.push(`El valor del campo "${msj[1]}" ya existe en la base de datos y no puede ser repetido.`);
        ...
        res.render("carreras/crear", { datos: carrera, errors: mensajes });
      });
  } catch (error) {
...
exports.actualizarCarrera_post = async function (req, res) {
  try {
    ...
    if (!carrera) {
      ...
      let resultado = await ModeloCarrera.update(carreraActualizada, {
        where: { id: idCarrera },
      });
